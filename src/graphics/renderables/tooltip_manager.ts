...
export class TooltipManager {
  ...
  // Método para configurar el GlobalContext
  public setGlobalContext(globalContext: GlobalContext) {
    this.globalContext = globalContext;
  }

  public attachTooltip(element: HTMLElement, key: string) {
    const tooltipsEnabled = this.globalContext?.get_enable_tooltips() ?? true; // Si no hay GlobalContext, asumir que están habilitadas
    if (key in this.tooltipsDictionary && tooltipsEnabled) {
      console.log(`Attaching tooltip to ${key}`);
