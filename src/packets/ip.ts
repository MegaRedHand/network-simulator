...
export class IPv4Packet {
    ...
    // 16 bits
    get totalLength() { return this.payload.toBytes().length + this.internetHeaderLength * 4 };
    ...
    // 16 bits
    // TODO: compute
    get headerChecksum() { return this.computeChecksum() };
    ...
    toBytes({ withChecksum = true, withPayload = true }: { withChecksum?: boolean, withPayload?: boolean }) {
        let checksum = 0;
        if (withChecksum) {
